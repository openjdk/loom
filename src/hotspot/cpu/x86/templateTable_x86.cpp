...
static void find_or_allocate_monitor(InterpreterMacroAssembler* _masm,
                                     Register obj, Register rtop, Register rbot, Register rmon)
{
  ...
  assert_different_registers(obj, rtop, rbot, rmon);
  ...
  // find a free slot in the monitor block (result in rmon)
  {
    ...
    __ cmpptr(obj, Address(rtop, BasicObjectLock::obj_offset()));
...
static void find_monitor(InterpreterMacroAssembler* _masm,
                         Register obj, Register rtop, Register rbot)
{
  const Address monitor_block_top(
        rbp, frame::interpreter_frame_monitor_block_top_offset * wordSize);
  const Address monitor_block_bot(
  ...
  assert_different_registers(obj, rtop, rbot);
...
void TemplateTable::monitorenter() {
  ...
  if (ObjectMonitorMode::java()) {
    ...
    find_or_allocate_monitor(_masm, rax, rtop, rbot, rmon); // result in rmon
...
void TemplateTable::monitorexit() {
  ...
  if (ObjectMonitorMode::java()) {
    ...
    find_monitor(_masm, rax, rtop, rbot); // result in rtop
    ...
    assert_different_registers(method, flags, rtop, rax);
