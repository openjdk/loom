...
stackChunkOop ContMirror::allocate_stack_chunk(int stack_size) {
  ...
  stackChunkOop result;
  if (_thread->as_Java_thread()->no_gc_during_handshake()) {
    result = (stackChunkOop)allocator.try_allocate();
  } else {
    Handle conth(_thread, _cont);
    result = (stackChunkOop)allocator.allocate();
...
JVM_ENTRY(jint, CONT_TryForceYield0(JNIEnv* env, jobject jcont, jobject jthread)) {
  ...
  class ForceYieldClosure : public HandshakeClosure {
    ...
    void do_thread(Thread* target) {
      // The handshake can be carried out by any JavaThread.
      assert (target->is_Java_thread(), "");
      JavaThread* thread = target->as_Java_thread();

      guarantee (Thread::current()->is_Java_thread(), "Thread: %s", Thread::current()->name());
      JavaThread* current = JavaThread::current();
      current->set_no_gc_during_handshake(true);
      ...
      current->set_no_gc_during_handshake(false);
    }

  public:
    ForceYieldClosure(jobject jcont) : HandshakeClosure("ContinuationForceYieldClosure"),
                                       _jcont(jcont), _result(-1) {}
    jint result() const { return _result; }
  };
  ForceYieldClosure fyc(jcont);

  // Grab the Heap_lock for eventual allocation.
  // It's the wrong thread but that doesn't matter.
  MutexLocker ml(THREAD, Heap_lock);
