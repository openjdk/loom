...
ContMirror::ContMirror(JavaThread* thread, oop cont)
 : _thread(thread), _entry(thread->cont_entry()), _cont(cont), 
#ifndef PRODUCT
  _tail(NULL),
...
public:
  ...
  Freeze(JavaThread* thread, ContMirror& mirror) :
    ...
    assert (thread->cont_entry()->entry_sp() == _cont.entrySP(), "");
  ...
  bool add_chunks_size() {
    ...
    for (oop chunk = _cont.tail(); chunk != (oop)NULL; chunk = jdk_internal_misc_StackChunk::parent(chunk)) {
      ...
      int size = (jdk_internal_misc_StackChunk::size(chunk) - jdk_internal_misc_StackChunk::sp(chunk) + frame_metadata) << LogBytesPerWord;
  ...
  freeze_result finalize(const frame& callee, hframe& caller, int* argsize_out) {
    ...
    assert (mode != mode_fast || _thread == NULL || _thread->cont_fastpath(), "");
    ...
    *argsize_out = argsize;
    ...
    if (mode == mode_fast && _thread != NULL && !_thread->cont_fastpath()) {
...
JRT_LEAF(int, Continuation::prepare_thaw(JavaThread* thread, bool return_barrier))
  ...
  oop cont = thread->cont_entry()->cont_raw(); // get_continuation(thread);
